...
import { X, Send, Paperclip, Loader2, ShieldCheck } from "lucide-react";
...
export default function DMChat({
  ...
  async function cargarMensajes() {
    ...
    if (!error) {
      const lista = (data as DMensaje[]) || [];
      setMensajes(lista);

      // Marcar como leídos los mensajes que me llegaron a mí
      const destinatarioPropio = esAdmin ? "ADMIN" : miUsuario;

      const idsSinLeer = lista
        .filter((m) => !m.leido && m.destinatario === destinatarioPropio && m.remitente !== miUsuario)
        .map((m) => m.id);
  ...
  async function enviar() {
    ...
    try {
      if (archivo) {
        ...
        const ruta = `${conversacionId}/${Date.now()}-${Math.random()
          .toString(36)
          .slice(2)}.${extension}`;
      ...
      <div className="w-full max-w-md rounded-2xl border border-zinc-700 bg-zinc-900 flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center justify-between border-b border-zinc-800 px-5 py-4">
          <div className="flex items-center gap-2 min-w-0">
            {esSoporte && <ShieldCheck size={18} className="text-emerald-500 shrink-0" />}
            <div className="min-w-0">
              <p className="text-xs text-zinc-500">
                {esSoporte ? "Soporte / Reclamo de premio" : "Chat"}
              </p>
              <p className="font-bold truncate">{otherLabel}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          {mensajes.length === 0 && (
            <p className="text-center text-zinc-500 text-sm mt-8">
              {esSoporte
                ? "Contanos qué necesitás. Si ganaste un torneo, decinos cuál y coordinamos la entrega del premio."
                : "Todavía no hay mensajes. ¡Mandá el primero!"}
            </p>
          )}

          {mensajes.map((m) => {
            const esMio = m.remitente === miUsuario && m.es_admin === esAdmin;
